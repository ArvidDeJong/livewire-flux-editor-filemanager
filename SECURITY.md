...
- **Laravel Filemanager and Flux.** Upload limits, file type checks and access to the `/filemanager` routes are configured in `config/lfm.php` (the `drag_drop` limits in `config/flux-filemanager.php` are a browser-side check only) and belong to [unisharp/laravel-filemanager](https://github.com/UniSharp/laravel-filemanager). Problems in the editor component itself belong to Flux Pro.
- **The demo pages.** `/darvis/editor-demo` and `/darvis/filemanager-checklist` are off by default (`flux-filemanager.demo_routes`) and have no authentication. Don't enable them in production. See [Installation](https://arviddejong.github.io/livewire-flux-editor-filemanager/installation.html#try-the-demo-pages-first-optional).
